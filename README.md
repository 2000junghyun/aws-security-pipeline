...
- AWS 보안 자동화를 위해 필요한 기능을 **컴포넌트화**하고, 디렉토리 단위로 분리한 **이벤트 기반 모듈형 침해 탐지·대응 파이프라인**
- 파이프라인 흐름: **Classifier → Responder → Notifier → Logger**
- 신규 위협(탐지 로직) 추가 시 **Classifier 확장** 중심으로 변경 비용을 최소화하도록 설계함

## Background

- 아키텍처 설계 단계에서 보안 요구사항을 강제하지 못하면 설정 누락 리스크가 발생 가능함
- 서비스별로 개별 배포되는 구조에서는 네트워크 분리, 접근 제어, 로그 수집 방식이 일관성이 떨어짐
- 보안 아키텍처와 코드 구조에 대한 기준이 없으면 신규 서비스/보안 기능 확장 시 비효율 발생함
...
| --- | --- | --- |
| `aws-classifier/` | Detect | 이벤트 입력을 기준으로 위협을 분류하고, 필요 시 대응/알림/기록 모듈을 호출 |
| `aws-responder/` | Respond | 분류 결과에 따라 차단·격리(예: Access Key 비활성화, 정책 제한, 인스턴스 종료 등) 작업을 수행 |
| `aws-notifier/` | Notify | 탐지/대응 이벤트를 사람이 확인할 수 있도록 SNS 기반 이메일 알림을 전송 |
| `aws-logger/` | Log | 이벤트 필드를 정규화해 **일관된 포맷의 구조화 로그**를 생성 |
...
- **Modular, Event-driven Design**
	- 탐지/대응/알림/기록을 디렉토리 단위 컴포넌트로 분리해 재사용성과 확장성을 높였음.
- **Classifier 중심 확장**
	- 신규 위협 유형 추가 시 Classifier 모듈(또는 하위 classifier)만 추가/확장하는 것을 기본 경로로 설계함.
- **Consistent Notification & Logging**
	- 알림/로깅은 별도 모듈로 분리해 파이프라인 전체에서 일관된 포맷과 라우팅을 유지함.

## Motivation / Impact

- 모듈형 침해 탐지·대응 파이프라인을 구축해 보안 기능 확장의 비용을 낮추고 운영 일관성을 높임.
- 대응(Responder)과 알림/기록(Notifier/Logger)을 표준화해, 탐지 소스가 늘어나도 운영 체계가 흔들리지 않도록 함.
